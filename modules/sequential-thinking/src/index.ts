...
import {
  ...
  ContextAnalyzer,
  ToolRecommender,
  defaultToolMetadataManager,
  defaultContextAnalyzer,
  defaultToolRecommender,
  ToolRecommendation
} from './tools/index.js';
 ...
 * 思考処理結果の型定義
 */
interface ThoughtProcessResult {
  thoughtNumber: number;
  totalThoughts: number;
  nextThoughtNeeded: boolean;
  branches: string[];
...
export class SequentialThinkingServer {
  ...
  private toolMetadataManager: ToolMetadataManager;
  ...
  private validateThoughtData(input: unknown): ThoughtData {
    ...
    return {
      ...
      includeToolRecommendations: data.includeToolRecommendations as boolean | undefined,
    };
  }
...
└${border}┘`;
   ...
   * 思考に基づいてツールを推奨する
   */
  private recommendToolsForThought(thought: string): ToolRecommendation[] {
    try {
      if (this.debugMode) {
  ...
  public processThought(input: unknown): { content: Array<{ type: string; text: string }>; isError?: boolean } {
    try {
      ...
      // 結果オブジェクトを作成
      const result: ThoughtProcessResult = {
