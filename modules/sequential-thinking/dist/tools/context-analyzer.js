...
export class ContextAnalyzer {
    ...
    analyzeThought(thought) {
        ...
        const uncertainties = lines
            .filter(line => line.includes('かもしれない') ||
            line.includes('可能性') ||
            line.includes('不明確') ||
            line.includes('不確か') ||
            line.includes('検討') ||
            ...
            .map(line => line.trim());
        // 次のステップが必要かどうかを判断
        const nextStepsNeeded = lines.some(line => line.includes('次のステップ') ||
            line.includes('今後の課題') ||
    ...
    analyzeContext(text) {
        ...
        const categories = Object.entries(this.categoryKeywords).map(([category, keywords]) => {
            // 完全一致と部分一致の両方を考慮
            const exactMatches = keywords.filter(keyword => normalizedText.includes(keyword.toLowerCase()));
            // 部分一致（キーワードの一部が含まれる場合）
            const partialMatches = keywords.filter(keyword => !exactMatches.includes(keyword) && // 完全一致は除外
            ...
            const confidence = ((exactMatches.length * exactMatchWeight) +
                (partialMatches.length * partialMatchWeight)) / keywords.length;
            return {
                category: category,
        ...
        // 全体的な信頼度を計算
        const overallConfidence = this.calculateOverallConfidence(categories[0]?.confidence || 0, contexts[0]?.confidence || 0, combinedKeywords.length);
        // 推奨アプローチを生成
        const suggestedApproach = this.generateSuggestedApproach(categories[0]?.category, contexts[0]?.context, complexityLevel, urgencyLevel);
        // デバッグ情報
        if (process.env.DEBUG === 'true') {
    ...
    hasPartialMatch(text, keyword) {
        ...
        return words.some(word => word.length > 3 && (word.includes(keyword) || keyword.includes(word)));
    }
    /**
     * 日本語の技術用語を抽出する
     */
    ...
    estimateComplexityLevel(text) {
        ...
        const complexityScore = (technicalTermFactor * 0.4) +
     ...
     * 全体的な信頼度を計算する
     */
    calculateOverallConfidence(topCategoryConfidence, topContextConfidence, keywordCount) {
     ...
     * 推奨アプローチを生成する
     */
    generateSuggestedApproach(topCategory, topContext, complexityLevel, urgencyLevel) {
