 ...
 */
import { defaultToolMetadataManager, ToolContext } from './metadata.js';
...
export class ToolRecommender {
    metadataManager;
    contextAnalyzer;
    constructor(metadataManager = defaultToolMetadataManager, contextAnalyzer = defaultContextAnalyzer) {
     ...
     * 思考データに基づいてツールを推奨する
     */
    recommendToolsForThought(thought, options = {}) {
        // 思考を分析
     ...
     * 分析結果に基づいてツールを推奨する
     */
    recommendTools(thoughtAnalysis, contextAnalysis, options = {}) {
        const { maxResults = 5, minScore = 0.2, // 最小スコアを0.3から0.2に下げる
        userExperienceLevel = 5, preferredCategories = [], excludedTools = [], includeUsageHints = true } = options;
        // 利用可能なすべてのツールを取得
        ...
        const scoredTools = filteredTools.map(tool => {
            const score = this.calculateToolScore(tool, thoughtAnalysis, contextAnalysis, userExperienceLevel, preferredCategories);
            const reasons = this.generateRecommendationReasons(tool, thoughtAnalysis, contextAnalysis, score);
     ...
     * ツールのスコアを計算する
     */
    calculateToolScore(tool, thoughtAnalysis, contextAnalysis, userExperienceLevel, preferredCategories) {
        // カテゴリマッチングスコア
        ...
        const keywordMatches = contextAnalysis.keywords
            .filter(kw => tool.keywords.some(toolKw => toolKw.toLowerCase().includes(kw.keyword.toLowerCase()) ||
        ...
        const weightedScore = (categoryScore * 0.15) +
            (contextScore * 0.15) +
     ...
     * 思考分析に基づく関連性スコアを計算する
     */
    calculateRelevanceScore(tool, thoughtAnalysis) {
        // 思考の主要な焦点とツールの説明の類似性を評価 - 改善版
        const mainFocusRelevance = this.enhancedTextSimilarity(thoughtAnalysis.mainFocus, tool.description + ' ' + tool.keywords.join(' '));
        ...
        const subTopicsRelevance = thoughtAnalysis.subTopics.length > 0
            ? thoughtAnalysis.subTopics.reduce((score, topic) => {
                const topicRelevance = this.enhancedTextSimilarity(topic, tool.description + ' ' + tool.keywords.join(' '));
        ...
        const questionsRelevance = thoughtAnalysis.questionsPosed.length > 0
            ? thoughtAnalysis.questionsPosed.reduce((score, question) => {
                const questionRelevance = this.enhancedTextSimilarity(question, tool.description + ' ' + tool.keywords.join(' '));
     ...
     */
    enhancedTextSimilarity(text1, text2) {
        if (!text1 || !text2)
            return 0;
        // 両方のテキストを正規化
        ...
        // 直接一致も考慮
        const directMatches = new Set(words1.filter(word => words2.includes(word)));
        // 部分一致と直接一致を組み合わせたスコア
     ...
     * 推奨理由を生成する
     */
    generateRecommendationReasons(tool, thoughtAnalysis, contextAnalysis, score) {
        const reasons = [];
        // カテゴリマッチング
     ...
     * 使用ヒントを生成する
     */
    generateUsageHint(tool, thoughtAnalysis, contextAnalysis) {
        // ツールの使用例からランダムに選択
        if (tool.usageExamples && tool.usageExamples.length > 0) {
            const randomExample = tool.usageExamples[Math.floor(Math.random() * tool.usageExamples.length)];
